...
async def test_get_job_debug_disabled_no_internal_data(client, monkeypatch):
    """GET /api/jobs/{id}?debug=true omits debug_data when DEBUG_ENDPOINTS_ENABLED is False."""
    from apps.api.routes import jobs as jobs_route

    monkeypatch.setattr(jobs_route.settings, "debug_endpoints_enabled", False)
    list_resp = await client.get("/api/jobs?per_page=1")
    assert list_resp.status_code == 200
    items = list_resp.json().get("items") or []
    if not items:
