...
def classify_jobs(self, job_ids: list[str] | dict | None = None):
    """
    ...
    job_ids: list of job ids, or dict from chain (e.g. {"job_ids": [...]}), or None for all SCORED.
    """
    # Accept chain output from score_jobs
    ...
    with log_context(task_name="classify_jobs"):
        metrics = get_metrics()
        ...
        with get_sync_session() as session:
            ...
            for job in jobs:
                ...
                classified_count += 1
                classified_job_ids.append(str(job.id))
    ...
    return {"classified": classified_count, "job_ids": classified_job_ids}
