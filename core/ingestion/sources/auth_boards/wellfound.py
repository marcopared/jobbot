...
class WellfoundSourceAdapter(BaseAuthBoardSourceAdapter):
    ...
    def extract_listings(self, *, html: str, page_record: AcquisitionRecord, **params: Any) -> Iterable[PublicBoardCandidate]:
        ...
        count = 0
        for card in soup.select("[data-testid='JobCard'], [data-test='JobCard']"):
...
def build_wellfound_adapter(
    *,
    policy: SourcePolicy | None = None,
    backend: AcquisitionBackend | None = None,
) -> WellfoundSourceAdapter:
    return WellfoundSourceAdapter(policy=policy, backend=backend)
